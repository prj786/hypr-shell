...
phase_services() {
    ...
    if command -v greetd >/dev/null 2>&1 || pkg_present greetd; then
        ...
        # an `env …` prefix, which greetd word-splits and mis-parses, nor a
        # service drop-in, whose env greetd may not pass through) is the only
        # reliable place to set the wlroots env for cage:
        #   WLR_NO_HARDWARE_CURSORS — fixes the inverted virtio-gpu pointer and
        #     the flaky `xe` cursor plane on Lunar Lake (both render a broken HW cursor).
        #   WLR_RENDERER_ALLOW_SOFTWARE (VM only) — lets cage fall back to software
        #     GL when the guest's virgl path can't allocate a surface.
        if [ "${DRY_RUN:-0}" = "1" ]; then info "would write /usr/local/bin/hypr-shell-greeter (cage wrapper)"
        else
